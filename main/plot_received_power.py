...
def plot_predicted_power_near_transmitter(frequency,transmitter):
    ...
    plt.xlabel("LAT, (degree)")
    plt.ylabel("LON, (degree)")
    plt.title(f"Predicted Received Power near {name} ({convert_to_si(frequency)}Hz)")
    cbar = plt.colorbar(fig)
    cbar.set_label('Received Power (dBm)')
...
def plot_differences(frequency,transmitter):
    ...
    plt.xlabel("LAT, (degree)")
    plt.ylabel("LON, (degree)")
    plt.title(f"Difference of Received Power with Calculated Value near {name} ({convert_to_si(frequency)}Hz)")
    cbar = plt.colorbar(fig)
    plt.clim(v_min, v_max)
    cbar.set_label('Received Power (dBm)')
    plt.savefig(f"main/IMGS/RP/DIFF/CALC_{name}_{convert_to_si(frequency)}Hz.png")
    ...
    plt.xlabel("LAT, (degree)")
    plt.ylabel("LON, (degree)")
    plt.title(f"Difference of Received Power with Predicted Value near {name} ({convert_to_si(frequency)}Hz)")
    cbar = plt.colorbar(fig)
    plt.clim(v_min, v_max)
    cbar.set_label('Received Power (dBm)')
    plt.savefig(f"main/IMGS/RP/DIFF/PRED_{name}_{convert_to_si(frequency)}Hz.png")
...
for transmitter in transmitters:
    for frequency in frequenct_list:
        plot_predicted_power_near_transmitter(frequency,transmitter)
        plot_expected_received_power_near_transmitter(frequency,transmitter)
